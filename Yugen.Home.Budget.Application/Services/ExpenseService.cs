...
namespace Yugen.Home.Budget.Application.Services
{
    public class ExpenseService
    {
        ...
        {
            var result = await _expenseRepository.ListAsync(0, 0, null, null);
        ...
        public async Task<PaginatedList<ResponseExpenseDto>> ListAsync(
            ...
            DateTime? end)
        {
            var result = await _expenseRepository.ListAsync(pageIndex, pageSize, start, end);
        ...
        {
            return await _expenseRepository.SumAsync(start, end);
        }

        public async Task<decimal> SumAccruedAsync(DateTime start, DateTime end)
        {
            return await _expenseRepository.SumAccruedAsync(start, end);
        }

        public async Task<List<ResponseExpenseGroupedByCategoryDto>> GroupedByCategoryAsync(DateTime start, DateTime end)
        {
            var response = await _expenseRepository.GroupedByCategoryAsync(start, end);
